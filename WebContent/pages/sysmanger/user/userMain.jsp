...
<style>
	...
	/* 弹出层 */
	#ggLog{width:300px;height:360px;background:#fff;position:absolute;left:0;right:0;top:0;bottom:0;margin:auto;border-radius:5px;box-shadow:0 0 5px #555;display:none;}
	...
	.errors{width:96%;margin:10px auto;background:#E1E6F6;}
</style>
</head>
<body>
	<!-- oaHead S -->
	...
	<div id="ggLog">
		...
		<form action="" method="post">
			...
			<label>
				<span>生&nbsp;&nbsp;&nbsp;日:</span><input type="date" placeholder="生日..." id="birthday" name="birthday" required="required"/>
			</label>
			<label>
	...
	<script>
		...
		$("#addUser").on('click',function(){
			...
			$("#ggLog").slideDown(500);
			//赋连接
			$("#ggLog").find("form").attr("action","<%=path%>/sysmanager/user/addUser.do");
		})
		...
		$(".update").on("click",function(){
			...
			$("#ggLog").slideDown(500);
			//赋连接
			$("#ggLog").find("form").attr("action","<%=path%>/sysmanager/user/upUserId.do");
		})
		//删除
			...
			$.tzAlert({
				...
				callback:function(ok){
					 if(ok){
						 $.ajax({
							type:"get",
							url:"<%=path%>/sysmanager/user/delUserJson.do",
							async:true,
							dataType:"json",
							data:{
								id:uid
